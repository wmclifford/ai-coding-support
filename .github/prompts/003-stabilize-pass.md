...
4. Load and validate `.ai/task-artifacts/<TASK-ID>/scaffold.yaml` against `schemas/scaffold-artifact.schema.v0.1.json`.
   Use `tools/validate_yaml.py` to perform the validation. If validation fails, present errors and stop.
5. Confirm the current git branch matches `branch_plan.initial_branch_name` from
   ...
   operator to check out the correct branch.
...
9. Validate the generated `stabilize.yaml` against `schemas/stabilize-artifact.schema.v0.1.json`. Use
