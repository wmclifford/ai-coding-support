...
- M3: Security & compliance gates — Add secrets scanning (gitleaks), dependency scanning, and optional policy checks (
...
- M1: Validation utilities — a simple CLI (preferably minimal-dependency) to validate schemas, task files, and artifact
  shapes locally (used in CI).
- M2: Template generator CLI — a helper to instantiate templates, optionally applying repo-level conventions (license,
  CODEOWNERS, initial task file). Can be a small Node or Python script.
...
- M1: Canonical docs — ensure `docs/governance/Goals.md`, `Conventions.md`, and `System.md` are complete and referenced
  by templates and CI.
